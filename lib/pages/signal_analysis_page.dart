...
import 'package:signal/pages/filter_page.dart';
...
class SignalAnalysisPage extends StatefulWidget {
  ...
  const SignalAnalysisPage({super.key, required this.filePath});
...
class _SignalAnalysisPageState extends State<SignalAnalysisPage> {
  ...
  Future<void> analyzeAudio() async {
    ...
    try {
      ...
      // Downsample data for large recordings
      final audioData =
          List.generate(bytes.length ~/ 2, (i) => bytes[i * 2] / 128.0);
      final downsampleFactor = max(1, audioData.length ~/ 1000);
      final downsampledData = List.generate(
          audioData.length ~/ downsampleFactor,
          (i) => audioData[i * downsampleFactor]);

      // Generate amplitude data
      amplitudeData = List.generate(
        downsampledData.length,
        (i) => FlSpot(i.toDouble() / sampleRate, downsampledData[i]),
      );

      // Compute FFT on a smaller segment
      final fftSegment = downsampledData.take(sampleRate).toList();
      final fftData = AudioFilter.computeFFT(fftSegment);
      final fftMagnitude = fftData
          .map((complex) =>
      ...
      frequencyMagnitudeData = List.generate(
        ...
        (i) => FlSpot(i * sampleRate / fftMagnitude.length, fftMagnitude[i]),
      );
  ...
  Widget _buildPortraitLayout() {
    return Column(
      children: [
        Expanded(flex: 4, child: _buildGraph()),
        _buildGraphControls(),
        _buildActionButtons(),
      ],
    );
  ...
  Widget _buildGraph() {
    ...
    final currentData =
        currentGraphIndex == 0 ? amplitudeData : frequencyMagnitudeData;
        ...
        child: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: () {
                setState(() =>
                    currentGraphIndex = (currentGraphIndex - 1).clamp(0, 1));
              },
            ),
            Expanded(
              child: Text(
...
//               label: Text(
//                   isProcessing ? 'Processing...' : 'Apply Band-Pass Filter'),
